...
namespace DeepSky.Haze
{
    ...
    public class DS_HazeView : MonoBehaviour
    {
        ...
        //private static string kDepthCmdBufferName = "DS_Haze_DepthDownsample";
        ...
        //private static string kHalfDepthTargetName = "DS_Haze_HalfDepthTarget";
        ...
        //private RenderTexture m_HalfResDepthTarget;
        ...
        //private CommandBuffer m_DepthDownsampleCmdBuffer;
        ...
        private int m_InterleavedOffsetIndex = 0;
        private int m_X;
        private int m_Y;
        ...
        public Light DirectLight
        {
            ...
            set
            {
                ...
                if (value)
                {
                    m_LightTransform = m_DirectLight.GetComponent<Transform>();
                }
            }
        }

        public RenderTexture RadianceTarget
        {
            get { return m_CurrentRadianceTarget; }
        }
        ...
        private bool CheckHasSystemSupport()
        {
            ...
            {
                Debug.LogError("DeepSky::DS_HazeView: Float render targets are not supported in Forward rendering with MSAA. Please disable MSAA or use Deferred.");
                enabled = false;
                return false;
            }
        ...
        {
            m_InterleavedOffsetIndex++;
            if (m_InterleavedOffsetIndex == 64)
            {
            ...
            switch (DS_HazeCore.Instance.HeightFalloff)
            {
                ...
                case DS_HazeCore.HeightFalloffType.Exponential:
                    float absY = Mathf.Abs(m_Transform.position.y);
            ...
            Vector4 betaParams = new Vector4(kMBetaS, ctx.m_HazeSecondaryScatteringRatio, kFBetaS, kFBetaE);
            ...
            //m_Material.SetTexture("_SamplingOffsets", m_InterleavedSampleTexture);
            ...
            if (m_DirectLight)
            {
                direction = -m_LightTransform.forward;
        ...
        void OnEnable()
        {
            ...
            //if (m_DepthDownsampleCmdBuffer != null)
            //{
            //    m_Camera.AddCommandBuffer(CameraEvent.AfterSkybox, m_DepthDownsampleCmdBuffer);
            //}
        ...
        /// <summary>
        /// Does the camera have the depth downsampling command buffer?
        /// </summary>
        /// <param name="foundCmd"></param>
        /// <returns></returns>
        //private bool CameraHasDepthDownSampleCmdBuffer(out CommandBuffer foundCmd)
        //{
        //    CommandBuffer[] cmdBuffers = m_Camera.GetCommandBuffers(CameraEvent.AfterSkybox);

        //    foreach (CommandBuffer cmd in cmdBuffers)
        //    {
        //        if (cmd.name == kDepthCmdBufferName)
        //        {
        //            foundCmd = cmd;
        //            return true;
        //        }
        ...
        {
            m_X = m_Camera.pixelWidth / 2;
            m_Y = m_Camera.pixelHeight / 2;
            ...
            //if (m_HalfResDepthTarget == null)
            //{
            //    CreateDepthTarget(kHalfDepthTargetName, out m_HalfResDepthTarget, true);
            //}
            //else
            //{
            //    if (m_HalfResDepthTarget.width != m_X || m_HalfResDepthTarget.height != m_Y)
            //    {
            //        DestroyImmediate(m_HalfResDepthTarget);
            //        CreateDepthTarget(kHalfDepthTargetName, out m_HalfResDepthTarget, true);
            //        if (m_DepthDownsampleCmdBuffer != null)
            //        {
            //            m_DepthDownsampleCmdBuffer.Dispose();
            //            m_DepthDownsampleCmdBuffer = null;
            //        }
            //    }
            //}
        ...
        void OnDisable()
        {
            ...
            //    if (cmd.name == kDepthCmdBufferName)
            //    {
            //        m_Camera.RemoveCommandBuffer(CameraEvent.AfterSkybox, cmd);
            //        break;
            //    }
            //}
        ...
        void OnDestroy()
        {
            ...
            //if (m_HalfResDepthTarget)
            //{
            //    m_HalfResDepthTarget.Release();
            //    DestroyImmediate(m_HalfResDepthTarget);
            //    m_HalfResDepthTarget = null;
            //}

            //if (m_DepthDownsampleCmdBuffer != null)
            //{
            //    m_Camera.RemoveCommandBuffer(CameraEvent.AfterSkybox, m_DepthDownsampleCmdBuffer);
            //    m_DepthDownsampleCmdBuffer.Dispose();
            //    m_DepthDownsampleCmdBuffer = null;
            //}
        ...
        void OnPreRender()
        {
            ...
            RenderTextureFormat rtFormat = m_Camera.allowHDR ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.ARGB32;
            m_PerFrameRadianceTarget = RenderTexture.GetTemporary(m_X, m_Y, 0, rtFormat);
            ...
            m_ClearRadianceCmdBuffer.ClearRenderTarget(false, true, Color.black);
            ...
            else
            {
                ...
                Vector3 vpC = m_Transform.forward * farClip;
                Vector3 vpR = m_Transform.right * dX * farClip;
                Vector3 vpU = m_Transform.up * dY * farClip;
                ...
                if (m_DirectLight)
                {
                    ...
                    m_DirectionalLightCmdBuffer.Blit(BuiltinRenderTextureType.None, m_PerFrameRadianceTarget, m_Material, 0);
                }
            }
            ...
            core.GetRenderLightVolumes(m_Transform.position, m_PerFrameLightVolumes, m_PerFrameShadowLightVolumes);
            ...
            {
                lv.SetupMaterialPerFrame(viewProjMtx, m_Camera.worldToCameraMatrix, m_Transform, DS_HazeCore.kOffsetSequence[m_InterleavedOffsetIndex]);

                // Add this light to this camera's command buffer.
                lv.AddLightRenderCommand(m_Transform, m_RenderNonShadowVolumes);
            }
            foreach (DS_HazeLightVolume lv in m_PerFrameShadowLightVolumes)
            {
                ...
                lv.FillLightCommandBuffer(m_PerFrameRadianceTarget, m_Transform);
        ...
        {
            RenderTexture tmpRad = RenderTexture.GetTemporary(m_X, m_Y, 0, m_Camera.allowHDR ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.ARGB32);
            RenderTexture tmpHalfDepth = RenderTexture.GetTemporary(m_X, m_Y, 0, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear, 1);

            // Down-sample the depth buffer.
            Graphics.Blit(null, tmpHalfDepth, m_Material, 5);
            //Shader.SetGlobalTexture("_HalfResDepth", m_HalfResDepthTarget);
            m_Material.SetTexture("_HalfResDepth", tmpHalfDepth);

            // Bi-lateral Gaussian gather pass.
            Graphics.Blit(m_PerFrameRadianceTarget, tmpRad, m_Material, 1);
            Graphics.Blit(tmpRad, m_PerFrameRadianceTarget, m_Material, 2);
            ...
            if (dest == null)
            {
                ...
                if (m_TemporalReprojection)
                {
                    ...
                    BlitToMRT(src, mrts1, m_Material, 3);
                }
                else
                {
                    Graphics.Blit(src, tmpDest, m_Material, 3);
                }
            ...
            else
            {
                ...
                {
                    Graphics.Blit(src, dest, m_Material, 3);
                }
            }
            ...
            if (m_TemporalReprojection)
            {
                Graphics.Blit(src, m_PreviousDepthTarget, m_Material, 4);
